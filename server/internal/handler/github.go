	...
	if h.workspaceAutoLinkPRsEnabled(ctx, wsID) {
		...
		if state == "merged" || state == "closed" {
			...
			for _, issue := range reevalIssues {
				...
				// custom terminal status counts as terminal here. (MUL-6243)
				if s := resolver.Effective(ctx, h.issueStatusCatalog(), issue.Status); s == "done" || s == "cancelled" || issue.Status == "archive" {
					continue
				}
