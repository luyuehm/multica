...
在**创建任务**模式下，可以设置**任务标题模板**和**任务正文模板**，让每次自动创建的任务都遵循统一结构。正文模板会替换原始描述作为任务正文骨架；`{{date}}` 渲染触发日期（按触发器时区，`YYYY-MM-DD`），`{{description}}` 渲染自动化自身的工作指令。模板保存时会拒绝其他任何 `{{...}}` token。未配置正文模板时，任务正文仍为描述原文加系统页脚——行为与之前完全一致。
