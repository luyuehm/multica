...
In **Create issue** mode you can set an **issue title template** and an **issue body template** so every auto-created issue follows a standard structure. The body template becomes the issue body skeleton in place of the raw description; `{{date}}` renders the trigger date (in the trigger timezone, `YYYY-MM-DD`) and `{{description}}` renders the autopilot's own instruction. Any other `{{...}}` token is rejected when the template is saved. When no body template is set, the issue body is the description verbatim plus the system footer — the behavior is unchanged.
